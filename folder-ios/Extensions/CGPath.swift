...
private enum Segment {
    ...
    var end: CGPoint {
        switch self {
        case .line(start: _, end: let end), let .quad(_, end), let .cubic(_, _, end):
            return end
        }
    }
}
...
public extension CGPath {
    ...
    func copy(roundingCornersToRadius radius: CGFloat) -> CGPath {
...
private extension CGMutablePath {
    func append(_ subpath: Subpath, withCornerRadius radius: CGFloat) {
        var priorSegment: Segment?
        ...
        if
            subpath.isClosed,
            case let .line(start: lastStart, end: lastEnd) = subpath.segments.last!,
...
private extension CGPoint {
    static func midpoint(_ p0: Self, _ p1: Self) -> Self {
...
private extension CGAffineTransform {
    ...
    static func standardizing(origin: CGPoint, unit: CGPoint) -> Self? {
...
private func clamp(r: CGFloat, under p: CGPoint) -> CGFloat {
    // Since `r` is given,
